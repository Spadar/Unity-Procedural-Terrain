...
namespace LibNoise
{
    ...
    public class Noise2D : IDisposable
    {
        ...
        public float this[int x, int y, bool isCropped]
        {
            get
            {
                if (isCropped)
        ...
        public float[,] GetNormalizedData(bool isCropped, int xCrop, int yCrop)
        {
        ...
        public float[,] GetData(bool isCropped, int xCrop, int yCrop, bool isNormalized)
        {
            int width, height;
            float[,] data;
            if (isCropped)
        ...
        {
			var x = System.Math.Cos(angle * Mathf.Deg2Rad);
            var y = height;
			var z = System.Math.Sin(angle * Mathf.Deg2Rad);
        ...
        {
			var r = System.Math.Cos(Mathf.Deg2Rad * lat);
			return _generator.GetValue(r * System.Math.Cos(Mathf.Deg2Rad * lon), System.Math.Sin(Mathf.Deg2Rad * lat),
			                           r * System.Math.Sin(Mathf.Deg2Rad * lon));
        }
